...
async function runBuild() {
  ...
  try {
    ...
    buildSpinner.buildSuccess()
    logger.info({ written: res }, 'Build finished')
  }
  catch (e) {
    buildSpinner.buildError()
    logger.error({ err: e }, 'Build failed')
    buildSpinner.scriptError()
    process.exitCode = 1
  }
}

async function main() {
  buildSpinner.scriptStart()
  try {
    if (CMD === 'build') {
      await runBuild()
      buildSpinner.scriptSuccess()
    }
    else if (CMD === 'dev') {
      buildSpinner.watchStart()
      await runBuild()
